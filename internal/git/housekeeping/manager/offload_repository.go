...
func (m *RepositoryManager) OffloadRepository(ctx context.Context, repo *localrepo.Repo, cfg config.OffloadHouseKeepingConfig) error {
	...
	if err := m.maybeStartTransaction(ctx, repo, func(ctx context.Context, tx storage.Transaction, repo *localrepo.Repo) error {
		...
		return fmt.Errorf("offloading repository: %w", err)
	}
...
// TODO we need to pay attention to how rollback when there is error during the operation
// we don have transaction, so it is hard to rollback the change. We need a best effort based
// approach to prevent corrupted/unfinished repository
