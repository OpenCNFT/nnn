...
func BlobOffloadPath(repo Repository) string {
	x := repo.GetGitAlternateObjectDirectories()
	log.Info("I have git alternate dir: %v", x)
...
func BlobOffloadPathOnDevice(repositoryID int64) string {
	return deriveDiskPath(fmt.Sprintf("/tmp/%s", praefectBlobOffloadPathPrefix), repositoryID)
}
