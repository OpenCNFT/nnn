...
func TestRegistry(t *testing.T) {
	...
	tests := []struct {
		name           string
		action         func(*Registry) []*Waiter
		expectedEvents []EventID
	}{
		{
			name: "Register and Remove single event",
			action: func(r *Registry) []*Waiter {
				waiter := r.Register()
				require.True(t, r.Untrack(waiter.ID))
		...
		},
		{
			...
			action: func(r *Registry) []*Waiter {
				w1 := r.Register()
				w2 := r.Register()
				require.True(t, r.Untrack(w1.ID))
				require.True(t, r.Untrack(w2.ID))
				return []*Waiter{w1, w2}
			},
		...
		},
		{
			...
			action: func(r *Registry) []*Waiter {
				w1 := r.Register()
				w2 := r.Register()
				require.True(t, r.Untrack(w2.ID)) // Removing the second one first
				require.True(t, r.Untrack(w1.ID)) // Then the first one
				return []*Waiter{w1, w2}
			},
			expectedEvents: []EventID{1, 2},
		},
		{
			...
			action: func(r *Registry) []*Waiter {
				require.False(t, r.Untrack(1234))
...
func TestRegistry_ConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func() {
			...
			for i := range waiters {
				assert.True(t, registry.Untrack(i))
			}
		}()
	}
