	...
	}{
		{
			...
			setup: func(t *testing.T) setupData {
				...
				return setupData{
					...
					verify: func() {
						actualCommit, err := gittest.ResolveRevisionAPI(t, ctx, commitClient, repo, "refs/tmp/fetch-source-branch-test^{commit}")
		...
		},
		{
			...
			setup: func(t *testing.T) setupData {
				...
				return setupData{
					...
					verify: func() {
						actualCommit, err := gittest.ResolveRevisionAPI(t, ctx, commitClient, repo, git.DefaultRef.String()+"^{commit}")
						require.NoError(t, err)
		...
		},
		{
			...
			setup: func(t *testing.T) setupData {
				...
				return setupData{
					...
					verify: func() {
						actualCommit, err := gittest.ResolveRevisionAPI(t, ctx, commitClient, sourceRepo, "refs/tmp/fetch-source-branch-test^{commit}")
						require.NoError(t, err)
						require.Equal(t, commitID, actualCommit)
					},
				}
